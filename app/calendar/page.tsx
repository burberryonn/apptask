    ...
    <div className="space-y-4 rounded-2xl bg-card p-4 shadow-soft">
      <div className="flex items-center justify-between">
        ...
        <div className="space-x-2">
          <button className={`rounded-lg px-3 py-1 ${mode === 'month' ? 'bg-primary text-white' : 'bg-accent'}`} onClick={() => setMode('month')}>Month</button>
          <button className={`rounded-lg px-3 py-1 ${mode === 'week' ? 'bg-primary text-white' : 'bg-accent'}`} onClick={() => setMode('week')}>Week</button>
        </div>
      </div>
      <p className="text-sm text-slate-500">Selected day: {format(day, 'PPPP')}</p>
      ...
      <section>
        <h2 className="text-xl font-semibold">Tasks ({dayTasks.length})</h2>
        {dayTasks.map((task) => (
